...
public class TravelZonesSimpleImpl implements TravelZones{
    ...
    public TravelZonesSimpleImpl(Scenario scenario, int nx, int ny) {
    	this(scenario, nx, ny, null);
    }

    public TravelZonesSimpleImpl(Scenario scenario, int nx, int ny, String activityType) {
        this.nx = nx;
        this.ny = ny;
        ...
        for ( Id<ActivityFacility> facilityId : scenario.getActivityFacilities().getFacilities().keySet() ) {
            ...
            if (activityType == null || scenario.getActivityFacilities().getFacilities().get(facilityId)
            		.getActivityOptions().containsKey(activityType)) {
	            if ( x > this.max_x) {
	                this.max_x = x;
	            }
	            if ( y > this.max_y) {
